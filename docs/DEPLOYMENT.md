...
| PP-OCRv3 text (default `auto`) | nothing: the 2.4 MB model downloads to `EZRA_MODEL_CACHE` on first render | Falls back to the morphological detector offline; `EZRA_TEXT_DETECTOR=morph` to skip it |
